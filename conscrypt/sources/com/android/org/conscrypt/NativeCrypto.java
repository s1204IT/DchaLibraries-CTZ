...
public final class NativeCrypto {
    ...
    static native void BIO_write(long j, byte[] bArr, int i, int i2) throws IndexOutOfBoundsException, IOException;

    static native int ECDH_compute_key(byte[] bArr, int i, NativeRef.EVP_PKEY evp_pkey, NativeRef.EVP_PKEY evp_pkey2) throws IndexOutOfBoundsException, InvalidKeyException;
    ...
    static native int ENGINE_SSL_read_BIO_heap(long j, NativeSsl nativeSsl, long j2, byte[] bArr, int i, int i2, SSLHandshakeCallbacks sSLHandshakeCallbacks) throws IndexOutOfBoundsException, IOException;
    ...
    static native int ENGINE_SSL_write_BIO_heap(long j, NativeSsl nativeSsl, long j2, byte[] bArr, int i, int i2, SSLHandshakeCallbacks sSLHandshakeCallbacks) throws IndexOutOfBoundsException, IOException;
    ...
    static native int EVP_AEAD_CTX_open(long j, byte[] bArr, int i, byte[] bArr2, int i2, byte[] bArr3, byte[] bArr4, int i3, int i4, byte[] bArr5) throws IndexOutOfBoundsException, BadPaddingException;

    static native int EVP_AEAD_CTX_seal(long j, byte[] bArr, int i, byte[] bArr2, int i2, byte[] bArr3, byte[] bArr4, int i3, int i4, byte[] bArr5) throws IndexOutOfBoundsException, BadPaddingException;
    ...
    static {
        ...
        int i = length / 2;
        SUPPORTED_CIPHER_SUITES = new String[i + 2];
        for (int i2 = EXTENSION_TYPE_NON_CRITICAL; i2 < length; i2 += 2) {
            String strCipherSuiteToJava = cipherSuiteToJava(strArr2[i2]);
            SUPPORTED_CIPHER_SUITES[i2 / 2] = strCipherSuiteToJava;
            SUPPORTED_CIPHER_SUITES_SET.add(strCipherSuiteToJava);
            SUPPORTED_LEGACY_CIPHER_SUITES_SET.add(strArr2[i2 + 1]);
        }
        SUPPORTED_CIPHER_SUITES[i] = TLS_EMPTY_RENEGOTIATION_INFO_SCSV;
        SUPPORTED_CIPHER_SUITES[i + 1] = TLS_FALLBACK_SCSV;
        HAS_AES_HARDWARE = EVP_has_aes_hardware() != 1 ? EXTENSION_TYPE_NON_CRITICAL : true;
        if (HAS_AES_HARDWARE) {
    ...
    private static int X509_NAME_hash(X500Principal x500Principal, String str) {
        try {
            ...
            return ((bArrDigest[3] & 255) << 24) | ((bArrDigest[1] & 255) << 8) | ((bArrDigest[EXTENSION_TYPE_NON_CRITICAL] & 255) << EXTENSION_TYPE_NON_CRITICAL) | ((bArrDigest[2] & 255) << 16);
    ...
    static void setEnabledProtocols(long j, NativeSsl nativeSsl, String[] strArr) {
        ...
        for (int i = EXTENSION_TYPE_NON_CRITICAL; i < length; i++) {
            ...
            if (str.equals(SUPPORTED_PROTOCOL_TLSV1)) {
                ...
                j2 &= -134217729;
                j3 |= 134217728;
            } else if (!str.equals(OBSOLETE_PROTOCOL_SSLV3)) {
                throw new IllegalStateException();
            }
        }
        SSL_set_options(j, nativeSsl, j2);
        SSL_clear_options(j, nativeSsl, j3);
    }
    ...
    static void setEnabledCipherSuites(long j, NativeSsl nativeSsl, String[] strArr) {
        checkEnabledCipherSuites(strArr);
        ArrayList arrayList = new ArrayList();
        for (int i = EXTENSION_TYPE_NON_CRITICAL; i < strArr.length; i++) {
            String str = strArr[i];
