...
public class OpenSSLSignatureRawECDSA extends SignatureSpi {
    ...
    protected byte[] engineSign() throws SignatureException {
        ...
        int iECDSA_size = NativeCrypto.ECDSA_size(this.key.getNativeRef());
        byte[] bArr2 = new byte[iECDSA_size];
        try {
            try {
                int iECDSA_sign = NativeCrypto.ECDSA_sign(this.buffer.toByteArray(), bArr2, this.key.getNativeRef());
                if (iECDSA_sign < 0) {
                    throw new SignatureException("Could not compute signature.");
                }
                if (iECDSA_sign != iECDSA_size) {
                    bArr = new byte[iECDSA_sign];
                    System.arraycopy(bArr2, 0, bArr, 0, iECDSA_sign);
                } else {
                    bArr = bArr2;
                }
                return bArr;
