...
public final class OpenSSLX509Certificate extends X509Certificate {
            ...
            try {
                ...
                for (int i = 0; i < jArrD2i_PKCS7_bio.length; i++) {
                    if (jArrD2i_PKCS7_bio[i] != 0) {
                        arrayList.add(new OpenSSLX509Certificate(jArrD2i_PKCS7_bio[i]));
                    }
                }
                return arrayList;
            } catch (Exception e) {
            ...
            try {
                ...
                for (int i = 0; i < jArrPEM_read_bio_PKCS7.length; i++) {
                    if (jArrPEM_read_bio_PKCS7[i] != 0) {
                        arrayList.add(new OpenSSLX509Certificate(jArrPEM_read_bio_PKCS7[i]));
                    }
                }
                return arrayList;
            } catch (Exception e) {
    ...
    private void verifyOpenSSL(OpenSSLKey openSSLKey) throws SignatureException, NoSuchAlgorithmException, InvalidKeyException, CertificateException {
        try {
    ...
    private void verifyInternal(PublicKey publicKey, String str) throws NoSuchAlgorithmException, SignatureException, InvalidKeyException, CertificateException, NoSuchProviderException {
        Signature signature;
        if (str == null) {
