...
public enum DcFailCause {
    ...
    MTK_PDP_FAIL_FALLBACK_RETRY(-1000),
    MTK_DUE_TO_REACH_RETRY_COUNTER(3599),
    MTK_TCM_ESM_TIMER_TIMEOUT(3910),
    ...
    public boolean isRestartRadioFail(Context context, int i) {
        CarrierConfigManager carrierConfigManager;
        PersistableBundle configForSubId;
        if (this == REGULAR_DEACTIVATION && (carrierConfigManager = (CarrierConfigManager) context.getSystemService("carrier_config")) != null && (configForSubId = carrierConfigManager.getConfigForSubId(i)) != null) {
            return configForSubId.getBoolean("restart_radio_on_pdp_fail_regular_deactivation_bool");
        }
        return false;
    }
    ...
    public boolean isPermanentFailure(Context context, int i) {
        ...
        synchronized (sPermanentFailureCache) {
            HashSet<DcFailCause> hashSet = sPermanentFailureCache.get(Integer.valueOf(i));
            if (hashSet == null) {
                CarrierConfigManager carrierConfigManager = (CarrierConfigManager) context.getSystemService("carrier_config");
                if (carrierConfigManager != null && (configForSubId = carrierConfigManager.getConfigForSubId(i)) != null && (stringArray = configForSubId.getStringArray("carrier_data_call_permanent_failure_strings")) != null) {
                    hashSet = new HashSet<>();
                ...
                if (hashSet == null) {
                    hashSet = new HashSet<DcFailCause>() {
                        {
                ...
                sPermanentFailureCache.put(Integer.valueOf(i), hashSet);
            }
            zContains = hashSet.contains(this);
        }
        return zContains;
    }
