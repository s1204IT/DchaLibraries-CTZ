...
public abstract class WifiScannerImpl {
    public static final WifiScannerImplFactory DEFAULT_FACTORY = new WifiScannerImplFactory() {
        ...
        public WifiScannerImpl create(Context context, Looper looper, Clock clock) {
            ...
            String clientInterfaceName = wifiNative.getClientInterfaceName();
            if (TextUtils.isEmpty(clientInterfaceName)) {
                return null;
            }
            if (wifiNative.getBgScanCapabilities(clientInterfaceName, new WifiNative.ScanCapabilities())) {
                return new HalWifiScannerImpl(context, clientInterfaceName, wifiNative, wifiMonitor, looper, clock);
            }
            return new WificondScannerImpl(context, clientInterfaceName, wifiNative, wifiMonitor, new WificondChannelHelper(wifiNative), looper, clock);
        }
    };
