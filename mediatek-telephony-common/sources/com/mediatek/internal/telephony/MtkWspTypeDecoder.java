...
public class MtkWspTypeDecoder extends WspTypeDecoder {
    ...
    public MtkWspTypeDecoder(byte[] bArr) {
        super(bArr);
    }

    public void decodeHeaders(int i, int i2) {
        String strValueOf;
        String strValueOf2;
        this.mHeaders = new HashMap<>();
        int decodedDataLength = i;
        while (decodedDataLength < i + i2) {
            decodeHeaderFieldName(decodedDataLength);
            decodedDataLength += getDecodedDataLength();
            expandWellKnownHeadersName();
            int i3 = (int) this.mUnsigned32bit;
            if (this.mStringValue != null) {
                strValueOf = this.mStringValue;
            } else if (i3 >= 0) {
                strValueOf = String.valueOf(i3);
            }
            decodeHeaderFieldValues(decodedDataLength);
            decodedDataLength += getDecodedDataLength();
            int i4 = (int) this.mUnsigned32bit;
            if (this.mStringValue != null) {
                strValueOf2 = this.mStringValue;
            } else if (i4 >= 0) {
                strValueOf2 = String.valueOf(i4);
            }
            this.mHeaders.put(strValueOf, strValueOf2);
        }
    }

    public boolean decodeHeaderFieldName(int i) {
    ...
    public boolean decodeHeaderFieldValues(int i) {
        if (this.mWspData[i] == 31 && decodeUintvarInteger(i + 1)) {
            this.mStringValue = null;
            this.mDataLength++;
            return true;
        }
    ...
    public void expandWellKnownXWapApplicationIdName() {
        String str;
        try {
            int iIntValue = Integer.valueOf(this.mHeaders.get("X-Wap-Application-Id")).intValue();
            if (iIntValue != -1 && (str = WELL_KNOWN_X_WAP_APPLICATION_ID.get(Integer.valueOf(iIntValue))) != null) {
                this.mHeaders.put("X-Wap-Application-Id", str);
            }
        } catch (Exception e) {
        }
    }
