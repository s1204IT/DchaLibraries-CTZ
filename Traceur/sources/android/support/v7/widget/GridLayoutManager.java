...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    public void onInitializeAccessibilityNodeInfoForItem(RecyclerView.Recycler recycler, RecyclerView.State state, View host, AccessibilityNodeInfoCompat info) {
        ...
        int spanGroupIndex = getSpanGroupIndex(recycler, state, layoutParams.getViewLayoutPosition());
        if (this.mOrientation == 0) {
            info.setCollectionItemInfo(AccessibilityNodeInfoCompat.CollectionItemInfoCompat.obtain(layoutParams.getSpanIndex(), layoutParams.getSpanSize(), spanGroupIndex, 1, this.mSpanCount > 1 && layoutParams.getSpanSize() == this.mSpanCount, false));
        } else {
            info.setCollectionItemInfo(AccessibilityNodeInfoCompat.CollectionItemInfoCompat.obtain(spanGroupIndex, 1, layoutParams.getSpanIndex(), layoutParams.getSpanSize(), this.mSpanCount > 1 && layoutParams.getSpanSize() == this.mSpanCount, false));
        }
    }
    ...
    void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state, LinearLayoutManager.LayoutState layoutState, LinearLayoutManager.LayoutChunkResult result) {
        ...
        while (i3 < count2) {
            ...
            if (this.mOrientationHelper.getDecoratedMeasurement(view4) == maxSize) {
                ...
                if (this.mOrientation == 1) {
                    remainingSpan = remainingSpan3;
                    wSpec = getChildMeasureSpec(totalSpaceInOther, 1073741824, horizontalInsets, ((ViewGroup.LayoutParams) lp).width, false);
                    ...
                    hSpec = getChildMeasureSpec(totalSpaceInOther, 1073741824, verticalInsets, ((ViewGroup.LayoutParams) lp).height, false);
                }
    ...
    private void measureChild(View view, int otherDirParentSpecMode, boolean alreadyMeasured) {
        ...
        if (this.mOrientation == 1) {
            wSpec = getChildMeasureSpec(availableSpaceInOther, otherDirParentSpecMode, horizontalInsets, ((ViewGroup.LayoutParams) lp).width, false);
            hSpec = getChildMeasureSpec(this.mOrientationHelper.getTotalSpace(), getHeightMode(), verticalInsets, ((ViewGroup.LayoutParams) lp).height, true);
        } else {
            int wSpec2 = ((ViewGroup.LayoutParams) lp).height;
            hSpec = getChildMeasureSpec(availableSpaceInOther, otherDirParentSpecMode, verticalInsets, wSpec2, false);
            wSpec = getChildMeasureSpec(this.mOrientationHelper.getTotalSpace(), getWidthMode(), horizontalInsets, ((ViewGroup.LayoutParams) lp).width, true);
        }
