...
final class GridLayoutManager extends RecyclerView.LayoutManager {
    ...
    int getDecoratedMeasuredWidthWithMargin(View v) {
        LayoutParams lp = (LayoutParams) v.getLayoutParams();
        return getDecoratedMeasuredWidth(v) + ((ViewGroup.MarginLayoutParams) lp).leftMargin + ((ViewGroup.MarginLayoutParams) lp).rightMargin;
    }

    int getDecoratedMeasuredHeightWithMargin(View v) {
        LayoutParams lp = (LayoutParams) v.getLayoutParams();
        return getDecoratedMeasuredHeight(v) + ((ViewGroup.MarginLayoutParams) lp).topMargin + ((ViewGroup.MarginLayoutParams) lp).bottomMargin;
    }
    ...
    private void measureScrapChild(int position, int widthSpec, int heightSpec, int[] measuredDimension) {
        ...
        if (view != null) {
            ...
            int widthUsed = ((ViewGroup.MarginLayoutParams) p).leftMargin + ((ViewGroup.MarginLayoutParams) p).rightMargin + sTempRect.left + sTempRect.right;
            int heightUsed = ((ViewGroup.MarginLayoutParams) p).topMargin + ((ViewGroup.MarginLayoutParams) p).bottomMargin + sTempRect.top + sTempRect.bottom;
            int childWidthSpec = ViewGroup.getChildMeasureSpec(widthSpec, getPaddingLeft() + getPaddingRight() + widthUsed, ((ViewGroup.LayoutParams) p).width);
            int childHeightSpec = ViewGroup.getChildMeasureSpec(heightSpec, getPaddingTop() + getPaddingBottom() + heightUsed, ((ViewGroup.LayoutParams) p).height);
    ...
    void measureChild(View child) {
        ...
        int widthUsed = ((ViewGroup.MarginLayoutParams) lp).leftMargin + ((ViewGroup.MarginLayoutParams) lp).rightMargin + sTempRect.left + sTempRect.right;
        int heightUsed = ((ViewGroup.MarginLayoutParams) lp).topMargin + ((ViewGroup.MarginLayoutParams) lp).bottomMargin + sTempRect.top + sTempRect.bottom;
        if (this.mRowSizeSecondaryRequested == -2) {
        ...
        if (this.mOrientation == 0) {
            widthSpec = ViewGroup.getChildMeasureSpec(View.MeasureSpec.makeMeasureSpec(0, 0), widthUsed, ((ViewGroup.LayoutParams) lp).width);
            heightSpec = ViewGroup.getChildMeasureSpec(secondarySpec, heightUsed, ((ViewGroup.LayoutParams) lp).height);
        } else {
            heightSpec = ViewGroup.getChildMeasureSpec(View.MeasureSpec.makeMeasureSpec(0, 0), heightUsed, ((ViewGroup.LayoutParams) lp).height);
            widthSpec = ViewGroup.getChildMeasureSpec(secondarySpec, widthUsed, ((ViewGroup.LayoutParams) lp).width);
        }
    ...
    public void startSmoothScroll(RecyclerView.SmoothScroller smoothScroller) {
        ...
        if (smoothScroller.isRunning() && (smoothScroller instanceof GridLinearSmoothScroller)) {
            this.mCurrentSmoothScroller = smoothScroller;
    ...
    public void onRestoreInstanceState(Parcelable parcelable) {
        ...
        this.mFocusPosition = parcelable.index;
        this.mFocusPositionOffset = 0;
        this.mChildrenStates.loadFromBundle(parcelable.childStates);
        this.mFlag |= 256;
        requestLayout();
    }
