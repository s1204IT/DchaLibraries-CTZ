...
public class DOMASBuilderImpl extends DOMParserImpl implements DOMASBuilder {
    ...
    @Override
    public ASModel parseASURI(String uri) throws Exception {
        XMLInputSource source = new XMLInputSource(null, uri, null);
        return parseASInputSource(source);
    }

    @Override
    public ASModel parseASInputSource(LSInput is) throws Exception {
        XMLInputSource xis = dom2xmlInputSource(is);
        try {
    ...
    private void addGrammars(ASModelImpl model, XSGrammarBucket grammarBucket) {
        ...
        for (SchemaGrammar schemaGrammar : grammarList) {
            ASModelImpl newModel = new ASModelImpl();
            newModel.setGrammar(schemaGrammar);
