...
public class StructNlMsgHdr {
    ...
    public String toString() {
        return "StructNlMsgHdr{ nlmsg_len{" + this.nlmsg_len + "}, nlmsg_type{" + (BackupManagerConstants.DEFAULT_BACKUP_FINISHED_NOTIFICATION_RECEIVERS + ((int) this.nlmsg_type) + "(" + NetlinkConstants.stringForNlMsgType(this.nlmsg_type) + ")") + "}, nlmsg_flags{" + (BackupManagerConstants.DEFAULT_BACKUP_FINISHED_NOTIFICATION_RECEIVERS + ((int) this.nlmsg_flags) + "(" + stringForNlMsgFlags(this.nlmsg_flags) + ")") + ")}, nlmsg_seq{" + this.nlmsg_seq + "}, nlmsg_pid{" + this.nlmsg_pid + "} }";
    }
}
