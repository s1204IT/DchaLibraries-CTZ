...
public class MaterialShapeDrawable extends Drawable implements TintAwareDrawable {
    ...
    public void draw(Canvas canvas) {
        ...
        if (this.shapedViewModel != null) {
            ...
            canvas.drawRect(ContactPhotoManager.OFFSET_DEFAULT, ContactPhotoManager.OFFSET_DEFAULT, canvas.getWidth(), canvas.getHeight(), this.paint);
        }
    ...
    private void getCoordinatesOfCorner(int index, int width, int height, PointF pointF) {
        switch (index) {
            case 1:
                pointF.set(width, ContactPhotoManager.OFFSET_DEFAULT);
                break;
            case 2:
                pointF.set(width, height);
                break;
            case 3:
                pointF.set(ContactPhotoManager.OFFSET_DEFAULT, height);
                break;
            default:
                pointF.set(ContactPhotoManager.OFFSET_DEFAULT, ContactPhotoManager.OFFSET_DEFAULT);
                break;
        }
    }
    ...
    private float angleOfCorner(int index, int width, int height) {
        ...
        if (angle < ContactPhotoManager.OFFSET_DEFAULT) {
