...
public class SnackbarContentLayout extends LinearLayout implements ContentViewCallback {
    ...
    public void animateContentOut(int delay, int duration) {
        ...
        this.messageView.animate().alpha(ContactPhotoManager.OFFSET_DEFAULT).setDuration(duration).setStartDelay(delay).start();
        if (this.actionView.getVisibility() == 0) {
            this.actionView.setAlpha(1.0f);
            this.actionView.animate().alpha(ContactPhotoManager.OFFSET_DEFAULT).setDuration(duration).setStartDelay(delay).start();
        }
    }
}
