...
public final class Target {
    ...
    void normalizeWeights() {
        float sum = ContactPhotoManager.OFFSET_DEFAULT;
        int z = this.mWeights.length;
        for (int i = 0; i < z; i++) {
            float weight = this.mWeights[i];
            if (weight > ContactPhotoManager.OFFSET_DEFAULT) {
                sum += weight;
            }
        }
        if (sum != ContactPhotoManager.OFFSET_DEFAULT) {
            int z2 = this.mWeights.length;
            for (int i2 = 0; i2 < z2; i2++) {
                if (this.mWeights[i2] > ContactPhotoManager.OFFSET_DEFAULT) {
