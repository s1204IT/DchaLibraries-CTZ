...
public final class Palette {
    ...
    private Swatch getMaxScoredSwatchForTarget(Target target) {
        float maxScore = ContactPhotoManager.OFFSET_DEFAULT;
    ...
    private float generateScore(Swatch swatch, Target target) {
        float[] hsl = swatch.getHsl();
        float saturationScore = ContactPhotoManager.OFFSET_DEFAULT;
        float luminanceScore = ContactPhotoManager.OFFSET_DEFAULT;
        float populationScore = ContactPhotoManager.OFFSET_DEFAULT;
        int maxPopulation = this.mDominantSwatch != null ? this.mDominantSwatch.getPopulation() : 1;
        if (target.getSaturationWeight() > ContactPhotoManager.OFFSET_DEFAULT) {
            saturationScore = target.getSaturationWeight() * (1.0f - Math.abs(hsl[1] - target.getTargetSaturation()));
        }
        if (target.getLightnessWeight() > ContactPhotoManager.OFFSET_DEFAULT) {
