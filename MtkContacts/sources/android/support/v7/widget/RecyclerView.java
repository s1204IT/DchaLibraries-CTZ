...
public class RecyclerView extends ViewGroup implements NestedScrollingChild2 {
    ...
    private void pullGlows(float x, float overscrollX, float y, float overscrollY) {
        ...
        if (overscrollX < ContactPhotoManager.OFFSET_DEFAULT) {
            ensureLeftGlow();
            EdgeEffectCompat.onPull(this.mLeftGlow, (-overscrollX) / getWidth(), 1.0f - (y / getHeight()));
            invalidate = true;
        } else if (overscrollX > ContactPhotoManager.OFFSET_DEFAULT) {
            ensureRightGlow();
            EdgeEffectCompat.onPull(this.mRightGlow, overscrollX / getWidth(), y / getHeight());
            invalidate = true;
        }
        if (overscrollY < ContactPhotoManager.OFFSET_DEFAULT) {
            ensureTopGlow();
            EdgeEffectCompat.onPull(this.mTopGlow, (-overscrollY) / getHeight(), x / getWidth());
            invalidate = true;
        } else if (overscrollY > ContactPhotoManager.OFFSET_DEFAULT) {
            ensureBottomGlow();
            EdgeEffectCompat.onPull(this.mBottomGlow, overscrollY / getHeight(), 1.0f - (x / getWidth()));
            invalidate = true;
        }
        if (invalidate || overscrollX != ContactPhotoManager.OFFSET_DEFAULT || overscrollY != ContactPhotoManager.OFFSET_DEFAULT) {
    ...
    public View focusSearch(View focused, int direction) {
        ...
        if (canRunFocusFailure && (direction == 2 || direction == 1)) {
            ...
            if (this.mLayout.canScrollVertically()) {
                int absDir = direction == 2 ? BaseAccountType.Weight.NOTE : 33;
    ...
    public boolean onTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            ...
            case 1:
                ...
                if ((xvel == ContactPhotoManager.OFFSET_DEFAULT && yvel == ContactPhotoManager.OFFSET_DEFAULT) || !fling((int) xvel, (int) yvel)) {
                    setScrollState(0);
                }
                resetTouch();
                break;
        ...
        if (this.mLayout != null && !this.mLayoutFrozen && event.getAction() == 8) {
            if ((event.getSource() & 2) != 0) {
                ...
                if (this.mLayout.canScrollHorizontally()) {
                    ...
                    hScroll = ContactPhotoManager.OFFSET_DEFAULT;
                }
            } else if ((event.getSource() & 4194304) != 0) {
                float axisScroll = event.getAxisValue(26);
                if (this.mLayout.canScrollVertically()) {
                    vScroll2 = -axisScroll;
                    hScroll2 = ContactPhotoManager.OFFSET_DEFAULT;
                } else if (this.mLayout.canScrollHorizontally()) {
                    vScroll2 = ContactPhotoManager.OFFSET_DEFAULT;
                    hScroll2 = axisScroll;
                } else {
                    vScroll2 = ContactPhotoManager.OFFSET_DEFAULT;
                    hScroll2 = 0.0f;
                ...
                vScroll = ContactPhotoManager.OFFSET_DEFAULT;
                hScroll = 0.0f;
            }
            if (vScroll != ContactPhotoManager.OFFSET_DEFAULT || hScroll != ContactPhotoManager.OFFSET_DEFAULT) {
    ...
    public void draw(Canvas c) {
        ...
        if (this.mLeftGlow != null && !this.mLeftGlow.isFinished()) {
            ...
            c.rotate(270.0f);
            c.translate((-getHeight()) + padding, ContactPhotoManager.OFFSET_DEFAULT);
    ...
    public static abstract class SmoothScroller {
        ...
        private void onAnimation(int dx, int dy) {
            ...
            if (this.mPendingInitialRun && this.mTargetView == null && this.mLayoutManager != null && (pointF = computeScrollVectorForPosition(this.mTargetPosition)) != null && (pointF.x != ContactPhotoManager.OFFSET_DEFAULT || pointF.y != ContactPhotoManager.OFFSET_DEFAULT)) {
