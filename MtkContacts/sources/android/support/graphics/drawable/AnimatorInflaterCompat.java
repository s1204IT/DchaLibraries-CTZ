...
public class AnimatorInflaterCompat {
    ...
    private static PropertyValuesHolder getPVH(TypedArray styledAttributes, int valueType, int valueFromId, int valueToId, String propertyName) {
        ...
        if (valueType2 == 2) {
            ...
            if (getFloats) {
                if (hasFrom) {
                    if (fromType == 5) {
                        valueFrom2 = styledAttributes.getDimension(valueFromId, ContactPhotoManager.OFFSET_DEFAULT);
                    } else {
                        valueFrom2 = styledAttributes.getFloat(valueFromId, ContactPhotoManager.OFFSET_DEFAULT);
                    }
                    if (!hasTo) {
                        propertyValuesHolderOfFloat = PropertyValuesHolder.ofFloat(propertyName, valueFrom2);
                    } else {
                        if (toType3 == 5) {
                            valueTo6 = styledAttributes.getDimension(valueToId, ContactPhotoManager.OFFSET_DEFAULT);
                        } else {
                            valueTo6 = styledAttributes.getFloat(valueToId, ContactPhotoManager.OFFSET_DEFAULT);
                        }
                    ...
                    if (toType3 == 5) {
                        valueTo5 = styledAttributes.getDimension(valueToId, ContactPhotoManager.OFFSET_DEFAULT);
                    } else {
                        valueTo5 = styledAttributes.getFloat(valueToId, ContactPhotoManager.OFFSET_DEFAULT);
                    }
                ...
                if (fromType == 5) {
                    int valueFrom3 = (int) styledAttributes.getDimension(valueFromId, ContactPhotoManager.OFFSET_DEFAULT);
                ...
                if (!hasTo) {
                    ...
                    if (toType3 == 5) {
                        int valueTo7 = (int) styledAttributes.getDimension(valueToId, ContactPhotoManager.OFFSET_DEFAULT);
                        valueTo4 = valueTo7;
                        valueTo3 = 0;
                ...
                if (toType3 == 5) {
                    int valueTo8 = (int) styledAttributes.getDimension(valueToId, ContactPhotoManager.OFFSET_DEFAULT);
                    valueTo2 = valueTo8;
                    valueTo = 0;
                } else if (isColorType(toType3)) {
                    valueTo = 0;
    ...
    private static void setupPathMotion(Path path, ObjectAnimator oa, float precision, String propertyXName, String propertyYName) {
        ...
        float totalLength = ContactPhotoManager.OFFSET_DEFAULT;
        ArrayList<Float> contourLengths = new ArrayList<>();
        contourLengths.add(Float.valueOf(ContactPhotoManager.OFFSET_DEFAULT));
        do {
        ...
        float currentDistance = ContactPhotoManager.OFFSET_DEFAULT;
        int contourIndex = 0;
        int contourIndex2 = 0;
        while (true) {
            int i = contourIndex2;
    ...
    private static PropertyValuesHolder loadPvh(Context context, Resources res, Resources.Theme theme, XmlPullParser parser, String propertyName, int valueType) throws XmlPullParserException, IOException {
        ...
        if (keyframes2 != null) {
            ...
            if (size > 0) {
                ...
                if (endFraction < 1.0f) {
                    if (endFraction < ContactPhotoManager.OFFSET_DEFAULT) {
                ...
                if (startFraction != ContactPhotoManager.OFFSET_DEFAULT) {
                    if (startFraction < ContactPhotoManager.OFFSET_DEFAULT) {
                        firstKeyframe.setFraction(ContactPhotoManager.OFFSET_DEFAULT);
                    } else {
                        keyframes2.add(0, createNewKeyframe(firstKeyframe, ContactPhotoManager.OFFSET_DEFAULT));
                        count++;
                    }
                }
                ...
                while (i < count) {
                    ...
                    if (keyframe2.getFraction() >= f2) {
                        ...
                        while (true) {
                            ...
                            float fraction = keyframeArray[j2].getFraction();
                            f = ContactPhotoManager.OFFSET_DEFAULT;
                            if (fraction >= ContactPhotoManager.OFFSET_DEFAULT) {
                                break;
                            }
                            endIndex2 = j2;
                            j = j2 + 1;
    ...
    private static Keyframe loadKeyframe(Context context, Resources res, Resources.Theme theme, AttributeSet attrs, int valueType, XmlPullParser parser) throws XmlPullParserException, IOException {
        ...
        if (hasValue) {
            if (valueType != 3) {
                switch (valueType) {
                    case 0:
                        float value = TypedArrayUtils.getNamedFloat(a, parser, "value", 0, ContactPhotoManager.OFFSET_DEFAULT);
