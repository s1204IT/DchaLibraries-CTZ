...
public class ViewPager extends ViewGroup {
    ...
    void populate(int newCurrentItem) {
        ...
        if (curItem != null) {
            float extraWidthLeft = ContactPhotoManager.OFFSET_DEFAULT;
            int itemIndex = curIndex - 1;
            ItemInfo ii4 = itemIndex >= 0 ? this.mItems.get(itemIndex) : null;
            int clientWidth = getClientWidth();
            float leftWidthNeeded2 = clientWidth <= 0 ? ContactPhotoManager.OFFSET_DEFAULT : (2.0f - curItem.widthFactor) + (getPaddingLeft() / clientWidth);
            ...
            if (extraWidthRight < 2.0f) {
                ...
                if (clientWidth > 0) {
                    ...
                    rightWidthNeeded = ContactPhotoManager.OFFSET_DEFAULT;
                }
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            lp.childIndex = i;
            if (!lp.isDecor && lp.widthFactor == ContactPhotoManager.OFFSET_DEFAULT && (ii = infoForChild(child)) != null) {
    ...
    private void calculatePageOffsets(ItemInfo curItem, int curIndex, ItemInfo oldCurInfo) {
        ...
        int N = this.mAdapter.getCount();
        int width = getClientWidth();
        float marginOffset = width > 0 ? this.mPageMargin / width : ContactPhotoManager.OFFSET_DEFAULT;
        if (oldCurInfo != null) {
    ...
    private void recomputeScrollPosition(int width, int oldWidth, int margin, int oldMargin) {
        ...
        float scrollOffset = ii != null ? Math.min(ii.offset, this.mLastOffset) : ContactPhotoManager.OFFSET_DEFAULT;
    ...
    private boolean pageScrolled(int xpos) {
        if (this.mItems.size() == 0) {
            ...
            onPageScrolled(0, ContactPhotoManager.OFFSET_DEFAULT, 0);
    ...
    private boolean isGutterDrag(float x, float dx) {
        return (x < ((float) this.mGutterSize) && dx > ContactPhotoManager.OFFSET_DEFAULT) || (x > ((float) (getWidth() - this.mGutterSize)) && dx < ContactPhotoManager.OFFSET_DEFAULT);
    }
        ...
        if (action != 0) {
            if (action == 2) {
                ...
                if (activePointerId != -1) {
                    ...
                    if (dx != ContactPhotoManager.OFFSET_DEFAULT && !isGutterDrag(this.mLastMotionX, dx)) {
                        y = y2;
                    ...
                    if (xDiff > this.mTouchSlop && 0.5f * xDiff > yDiff) {
                        ...
                        setScrollState(1);
                        this.mLastMotionX = dx > ContactPhotoManager.OFFSET_DEFAULT ? this.mInitialMotionX + this.mTouchSlop : this.mInitialMotionX - this.mTouchSlop;
                        this.mLastMotionY = y;
                        setScrollingCacheEnabled(true);
                ...
                if (!this.mIsBeingDragged) {
                    ...
                    if (pointerIndex == -1) {
                        ...
                        if (xDiff > this.mTouchSlop && xDiff > yDiff) {
                            this.mIsBeingDragged = true;
                            requestParentDisallowInterceptTouchEvent(true);
                            this.mLastMotionX = x3 - this.mInitialMotionX > ContactPhotoManager.OFFSET_DEFAULT ? this.mInitialMotionX + this.mTouchSlop : this.mInitialMotionX - this.mTouchSlop;
    ...
    private ItemInfo infoForCurrentScrollPosition() {
        int width = getClientWidth();
        float marginOffset = ContactPhotoManager.OFFSET_DEFAULT;
        float scrollOffset = width > 0 ? getScrollX() / width : 0.0f;
        if (width > 0) {
            marginOffset = this.mPageMargin / width;
        }
        int lastPos = -1;
        float lastOffset = ContactPhotoManager.OFFSET_DEFAULT;
        float lastWidth = ContactPhotoManager.OFFSET_DEFAULT;
        boolean first = true;
    ...
    public static class LayoutParams extends ViewGroup.LayoutParams {
        ...
        public LayoutParams() {
            super(-1, -1);
            this.widthFactor = ContactPhotoManager.OFFSET_DEFAULT;
        }
