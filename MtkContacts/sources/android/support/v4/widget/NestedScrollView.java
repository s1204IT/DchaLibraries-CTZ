...
public class NestedScrollView extends FrameLayout implements NestedScrollingChild2, NestedScrollingParent2 {
    ...
    public boolean executeKeyEvent(KeyEvent event) {
        ...
        if (!zCanScroll) {
            ...
            View nextFocused = FocusFinder.getInstance().findNextFocus(this, currentFocused, BaseAccountType.Weight.NOTE);
            return (nextFocused == null || nextFocused == this || !nextFocused.requestFocus(BaseAccountType.Weight.NOTE)) ? false : true;
        }
        ...
        if (keyCode != 62) {
            switch (keyCode) {
                ...
                case 20:
                    if (!event.isAltPressed()) {
                        boolean handled3 = arrowScroll(BaseAccountType.Weight.NOTE);
                    } else {
                        boolean handled4 = fullScroll(BaseAccountType.Weight.NOTE);
                    }
                    break;
            }
            return false;
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        vtev.offsetLocation(ContactPhotoManager.OFFSET_DEFAULT, this.mNestedYOffset);
        switch (actionMasked) {
            case 0:
                z = true;
                if (getChildCount() == 0) {
            ...
            case 2:
                ...
                if (activePointerIndex == -1) {
                    ...
                    if (dispatchNestedPreScroll(0, deltaY, this.mScrollConsumed, this.mScrollOffset, 0)) {
                        ...
                        vtev.offsetLocation(ContactPhotoManager.OFFSET_DEFAULT, this.mScrollOffset[1]);
    ...
    public boolean onGenericMotionEvent(MotionEvent event) {
        if ((event.getSource() & 2) != 0 && event.getAction() == 8 && !this.mIsBeingDragged) {
            ...
            if (vscroll != ContactPhotoManager.OFFSET_DEFAULT) {
    ...
    protected boolean onRequestFocusInDescendants(int direction, Rect previouslyFocusedRect) {
        View nextFocus;
        if (direction == 2) {
            direction = BaseAccountType.Weight.NOTE;
        } else if (direction == 1) {
            direction = 33;
        }
    ...
    private void flingWithNestedDispatch(int velocityY) {
        ...
        if (!dispatchNestedPreFling(ContactPhotoManager.OFFSET_DEFAULT, velocityY)) {
            dispatchNestedFling(ContactPhotoManager.OFFSET_DEFAULT, velocityY, canFling);
            fling(velocityY);
        }
    }
    ...
    public void draw(Canvas canvas) {
        ...
        if (this.mEdgeGlowTop != null) {
            ...
            if (!this.mEdgeGlowBottom.isFinished()) {
                ...
                canvas.rotate(180.0f, width2, ContactPhotoManager.OFFSET_DEFAULT);
