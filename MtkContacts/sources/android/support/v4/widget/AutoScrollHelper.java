...
public abstract class AutoScrollHelper implements View.OnTouchListener {
    ...
    private float computeTargetVelocity(int direction, float coordinate, float srcSize, float dstSize) {
        ...
        if (value == ContactPhotoManager.OFFSET_DEFAULT) {
    ...
    private float getEdgeValue(float relativeValue, float size, float maxValue, float current) {
        float interpolated;
        float edgeSize = constrain(relativeValue * size, ContactPhotoManager.OFFSET_DEFAULT, maxValue);
    ...
    private float constrainEdgeValue(float current, float leading) {
        if (leading == ContactPhotoManager.OFFSET_DEFAULT) {
            return ContactPhotoManager.OFFSET_DEFAULT;
        }
        switch (this.mEdgeType) {
            ...
            case 1:
                if (current < leading) {
                    if (current >= ContactPhotoManager.OFFSET_DEFAULT) {
    ...
    void cancelTargetTouch() {
        ...
        MotionEvent cancel = MotionEvent.obtain(eventTime, eventTime, 3, ContactPhotoManager.OFFSET_DEFAULT, ContactPhotoManager.OFFSET_DEFAULT, 0);
    ...
    private static class ClampedScroller {
        ...
        private float getValueAt(long currentTime) {
            ...
            if (this.mStopTime < 0 || currentTime < this.mStopTime) {
                ...
                return 0.5f * AutoScrollHelper.constrain(elapsedSinceStart / this.mRampUpDuration, ContactPhotoManager.OFFSET_DEFAULT, 1.0f);
            }
            long elapsedSinceEnd2 = currentTime - this.mStopTime;
            return (1.0f - this.mStopValue) + (this.mStopValue * AutoScrollHelper.constrain(elapsedSinceEnd2 / this.mEffectiveRampDown, ContactPhotoManager.OFFSET_DEFAULT, 1.0f));
        }
