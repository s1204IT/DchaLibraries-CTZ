...
public class MultiSelectPhoneNumbersListAdapter extends MultiSelectEntryContactListAdapter {
    ...
    public void configureLoader(CursorLoader cursorLoader, long j) {
        ...
        if (isSearchMode()) {
            builderBuildUpon = ContactsContract.CommonDataKinds.Phone.CONTENT_FILTER_URI.buildUpon();
            ...
            builderBuildUpon = ContactsContract.CommonDataKinds.Phone.CONTENT_URI.buildUpon();
    ...
    protected void bindPhoneNumber(ContactListItemView contactListItemView, Cursor cursor) {
        ...
        if (!cursor.isNull(1)) {
            ...
            typeLabel = ContactsContract.CommonDataKinds.Phone.getTypeLabel(getContext().getResources(), i, string);
            if (ContactsPortableUtils.MTK_PHONE_BOOK_SUPPORT) {
                typeLabel = GlobalEnv.getSimAasEditor().getTypeLabel(i, string, (String) typeLabel, cursor.getInt(9));
            }
        } else {
            typeLabel = null;
        }
