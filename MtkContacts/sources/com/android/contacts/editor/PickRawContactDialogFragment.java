...
public class PickRawContactDialogFragment extends DialogFragment {
    ...
    private final class RawContactAccountListAdapter extends BaseAdapter {
        ...
        public View getView(int i, View view, ViewGroup viewGroup) {
            ...
            String string2 = this.mPreferences.getDisplayOrder() == 1 ? rawContact.displayName : rawContact.displayNameAlt;
            if (TextUtils.isEmpty(string2)) {
                string2 = this.mContext.getString(R.string.missing_name);
            }
            rawContactViewHolder.displayName.setText(string2);
            if (this.mRawContactsMetadata.isUserProfile && accountType.areContactsWritable()) {
                string = EditorUiUtils.getAccountHeaderLabelForMyProfile(this.mContext, AccountTypeManager.getInstance(PickRawContactDialogFragment.this.getContext()).getAccountInfoForAccount(new AccountWithDataSet(rawContact.accountName, rawContact.accountType, rawContact.accountDataSet)));
            } else if (GoogleAccountType.ACCOUNT_TYPE.equals(rawContact.accountType) && accountType.dataSet == null) {
                string = rawContact.accountName;
            } else {
                string = accountType.getDisplayLabel(this.mContext).toString();
            }
            rawContactViewHolder.accountName.setText(string);
            rawContactViewHolder.accountIcon.setImageDrawable(accountType.getDisplayIcon(this.mContext));
            ContactPhotoManager.getInstance(this.mContext).loadThumbnail(rawContactViewHolder.photo, rawContact.photoId, false, true, new ContactPhotoManager.DefaultImageRequest(string2, String.valueOf(rawContact.id), true));
            return view;
        }
