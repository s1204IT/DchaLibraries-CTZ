...
class VCardParserImpl_V30 extends VCardParserImpl_V21 {
    ...
    private void splitAndPutParam(VCardProperty vCardProperty, String str, String str2) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (cCharAt == '\"') {
                if (z) {
                    ...
                    if (sb != null) {
                        if (sb.length() > 0) {
                            Log.w("MTK_vCard", "Unexpected Dquote inside property.");
                        } else {
                            vCardProperty.addParameter(str, encodeParamValue(sb.toString()));
                        }
                    }
                ...
                Log.w("MTK_vCard", "Comma is used before actual string comes. (" + str2 + ")");
            } else {
                vCardProperty.addParameter(str, encodeParamValue(sb.toString()));
                sb = null;
            }
        ...
        if (z) {
            Log.d("MTK_vCard", "Dangling Dquote.");
        }
        if (sb != null) {
            if (sb.length() == 0) {
                Log.w("MTK_vCard", "Unintended behavior. We must not see empty StringBuilder at the end of parameter value parsing.");
            } else {
                vCardProperty.addParameter(str, encodeParamValue(sb.toString()));
            }
        }
    ...
    protected void handleAgent(VCardProperty vCardProperty) {
        if (!this.mEmittedAgentWarning) {
            Log.w("MTK_vCard", "AGENT in vCard 3.0 is not supported yet. Ignore it");
