...
public class AppBarLayout extends LinearLayout {
    ...
    public final int getTotalScrollRange() {
        ...
        while (true) {
            ...
            range += ((ViewGroup.MarginLayoutParams) lp).topMargin + childHeight + ((ViewGroup.MarginLayoutParams) lp).bottomMargin;
            if ((flags & 2) == 0) {
                i++;
            } else {
    ...
    int getDownNestedPreScrollRange() {
        ...
        for (int i = getChildCount() - 1; i >= 0; i--) {
            ...
            int flags = lp.scrollFlags;
            if ((flags & 5) == 5) {
                int range2 = range + ((ViewGroup.MarginLayoutParams) lp).topMargin + ((ViewGroup.MarginLayoutParams) lp).bottomMargin;
                if ((flags & 8) != 0) {
    ...
    int getDownNestedScrollRange() {
        ...
        while (true) {
            ...
            LayoutParams lp = (LayoutParams) child.getLayoutParams();
            int childHeight = child.getMeasuredHeight();
            int childHeight2 = childHeight + ((ViewGroup.MarginLayoutParams) lp).topMargin + ((ViewGroup.MarginLayoutParams) lp).bottomMargin;
            int flags = lp.scrollFlags;
            if ((flags & 1) == 0) {
                break;
            }
    ...
    public static class Behavior extends HeaderBehavior<AppBarLayout> {
        ...
        public boolean onMeasureChild(CoordinatorLayout parent, AppBarLayout child, int parentWidthMeasureSpec, int widthUsed, int parentHeightMeasureSpec, int heightUsed) {
            ...
            if (((ViewGroup.LayoutParams) lp).height == -2) {
        ...
        private int interpolateOffset(AppBarLayout layout, int offset) {
            ...
            while (true) {
                ...
                if (absOffset < child.getTop() || absOffset > child.getBottom()) {
                    ...
                    if ((flags & 1) != 0) {
                        childScrollableHeight = 0 + child.getHeight() + ((ViewGroup.MarginLayoutParams) childLp).topMargin + ((ViewGroup.MarginLayoutParams) childLp).bottomMargin;
                        if ((flags & 2) != 0) {
        ...
        public void onRestoreInstanceState(CoordinatorLayout parent, AppBarLayout appBarLayout, Parcelable parcelable) {
            if (parcelable instanceof SavedState) {
                super.onRestoreInstanceState(parent, appBarLayout, parcelable.getSuperState());
    ...
    public static class ScrollingViewBehavior extends HeaderScrollingViewBehavior {
        ...
        private void offsetChildAsNeeded(View child, View dependency) {
            ?? behavior = ((CoordinatorLayout.LayoutParams) dependency.getLayoutParams()).getBehavior();
            if (behavior instanceof Behavior) {
                ViewCompat.offsetTopAndBottom(child, (((dependency.getBottom() - child.getTop()) + ((Behavior) behavior).offsetDelta) + getVerticalLayoutGap()) - getOverlapPixelsForOffset(dependency));
            }
        }
        ...
        private static int getAppBarLayoutOffset(AppBarLayout abl) {
            ?? behavior = ((CoordinatorLayout.LayoutParams) abl.getLayoutParams()).getBehavior();
            if (behavior instanceof Behavior) {
