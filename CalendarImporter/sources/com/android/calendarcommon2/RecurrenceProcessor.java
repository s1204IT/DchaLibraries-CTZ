...
public class RecurrenceProcessor {
    ...
    private Time mIterator = new Time(DateTime.UTC);
    private Time mUntil = new Time(DateTime.UTC);
    private StringBuilder mStringBuilder = new StringBuilder();
    private Time mGenerated = new Time(DateTime.UTC);
    private DaySet mDays = new DaySet(SPEW);
    ...
    public long getLastOccurence(Time time, Time time2, RecurrenceSet recurrenceSet) throws DateException {
        ...
        if (recurrenceSet.rrules != null) {
            ...
            for (EventRecurrence eventRecurrence : recurrenceSet.rrules) {
                if (eventRecurrence.count != 0) {
                    ...
                    long millis = this.mIterator.toMillis(SPEW);
                    if (millis > j) {
                        j = millis;
                    }
                }
        ...
        long[] jArrExpand = expand(time, recurrenceSet, time.toMillis(SPEW), time2 != null ? time2.toMillis(SPEW) : -1L);
        if (jArrExpand.length == 0) {
            return 0L;
        }
        return jArrExpand[jArrExpand.length - 1];
    }
    ...
    public static class DaySet {
        ...
        private Time mTime = new Time(DateTime.UTC);
        private int mYear;
        ...
        boolean get(Time time, int i) {
            ...
            return RecurrenceProcessor.SPEW;
        }
    ...
    public long[] expand(Time time, RecurrenceSet recurrenceSet, long j, long j2) throws DateException {
        ...
        if (recurrenceSet.exrules != null) {
            ...
            int i2 = 0;
            while (i2 < length2) {
                expand(time, eventRecurrenceArr2[i2], jNormDateTimeComparisonValue2, j3, SPEW, treeSet);
                i2++;
                length2 = length2;
        ...
        try {
            ...
            switch (i41) {
                case 1:
                    ...
                    boolean z3 = (i41 >= 5 || (eventRecurrence.bydayCount <= 0 && eventRecurrence.bymonthdayCount <= 0)) ? SPEW : true;
                    i4 = i42;
                    ...
                    while (true) {
                        int i46 = i9 + 1;
                        if (i9 <= MAX_ALLOWED_ITERATIONS) {
                            Log.w(TAG, "Recurrence processing stuck with r=" + eventRecurrence + " rangeStart=" + j + " rangeEnd=" + j2);
                            return;
                        }
            ...
            Log.w(TAG, "DateException with r=" + eventRecurrence + " rangeStart=" + j + " rangeEnd=" + j2);
            throw e;
        } catch (RuntimeException e2) {
            Log.w(TAG, "RuntimeException with r=" + eventRecurrence + " rangeStart=" + j + " rangeEnd=" + j2);
            throw e2;
        }
    }
