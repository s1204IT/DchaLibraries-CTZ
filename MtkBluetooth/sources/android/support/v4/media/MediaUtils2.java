...
class MediaUtils2 {
    ...
    static MediaMetadataCompat convertToMediaMetadataCompat(MediaMetadata2 metadata2) {
        ...
        for (String key : bundle.keySet()) {
            ?? r5 = bundle.get(key);
            if (r5 instanceof CharSequence) {
                builder.putText(key, (CharSequence) r5);
            } else if (r5 instanceof Rating2) {
                builder.putRating(key, convertToRatingCompat(r5));
            } else if (r5 instanceof Bitmap) {
                builder.putBitmap(key, r5);
            } else if (r5 instanceof Long) {
                builder.putLong(key, r5.longValue());
            } else {
                skippedKeys.add(key);
            }
        }
        ...
        for (String key2 : skippedKeys) {
            ...
            if (r6 instanceof Float) {
                result.getBundle().putFloat(key2, r6.floatValue());
            } else if (MediaMetadata2.METADATA_KEY_EXTRAS.equals(r6)) {
                result.getBundle().putBundle(key2, (Bundle) r6);
            }
        }
        return result;
    }
