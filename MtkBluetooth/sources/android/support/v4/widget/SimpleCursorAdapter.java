...
public class SimpleCursorAdapter extends ResourceCursorAdapter {
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        ...
        for (int i = 0; i < count; i++) {
            ?? FindViewById = view.findViewById(to[i]);
            if (FindViewById != 0) {
                boolean bound = false;
                if (viewBinder != 0) {
                    bound = viewBinder.setViewValue(FindViewById, cursor, from[i]);
                }
                if (bound) {
                    continue;
                } else {
                    ...
                    if (FindViewById instanceof TextView) {
                        setViewText(FindViewById, text);
                    } else if (FindViewById instanceof ImageView) {
                        setViewImage(FindViewById, text);
                    } else {
