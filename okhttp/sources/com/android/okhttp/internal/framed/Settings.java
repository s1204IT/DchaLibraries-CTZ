...
public final class Settings {
    ...
    int getDownloadBandwidth(int i) {
        return (this.set & MAX_CONCURRENT_STREAMS) != 0 ? this.values[2] : i;
    }

    boolean getEnablePush(boolean z) {
        return ((this.set & MAX_CONCURRENT_STREAMS) != 0 ? this.values[2] : z ? 1 : 0) == 1;
    }

    int getRoundTripTime(int i) {
        return (this.set & CLIENT_CERTIFICATE_VECTOR_SIZE) != 0 ? this.values[ROUND_TRIP_TIME] : i;
    }

    int getMaxConcurrentStreams(int i) {
        return (this.set & 16) != 0 ? this.values[MAX_CONCURRENT_STREAMS] : i;
    }
    ...
    int getInitialWindowSize(int i) {
        return (this.set & 128) != 0 ? this.values[INITIAL_WINDOW_SIZE] : i;
    }

    int getClientCertificateVectorSize(int i) {
        return (this.set & 256) != 0 ? this.values[CLIENT_CERTIFICATE_VECTOR_SIZE] : i;
    }
