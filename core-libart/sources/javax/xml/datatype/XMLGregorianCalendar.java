...
public abstract class XMLGregorianCalendar implements Cloneable {
    ...
    public int hashCode() {
        XMLGregorianCalendar xMLGregorianCalendarNormalize;
        int timezone = getTimezone();
        if (timezone == Integer.MIN_VALUE) {
            timezone = 0;
        }
        if (timezone != 0) {
            xMLGregorianCalendarNormalize = normalize();
        } else {
            xMLGregorianCalendarNormalize = this;
        }
        return xMLGregorianCalendarNormalize.getYear() + xMLGregorianCalendarNormalize.getMonth() + xMLGregorianCalendarNormalize.getDay() + xMLGregorianCalendarNormalize.getHour() + xMLGregorianCalendarNormalize.getMinute() + xMLGregorianCalendarNormalize.getSecond();
    }
