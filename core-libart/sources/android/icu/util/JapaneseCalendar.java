...
public class JapaneseCalendar extends GregorianCalendar {
    ...
    protected int getDefaultMonthInYear(int i) {
        if (i == ERAS[internalGet(0, CURRENT_ERA) * 3]) {
            return ERAS[r0 + 1] - 1;
        }
    ...
    protected int getDefaultDayInMonth(int i, int i2) {
        int iInternalGet = internalGet(0, CURRENT_ERA) * 3;
        if (i == ERAS[iInternalGet] && i2 == ERAS[iInternalGet + 1] - 1) {
            return ERAS[iInternalGet + 2];
        }
    ...
    protected void handleComputeFields(int i) {
        ...
        if (iInternalGet > ERAS[ERAS.length - 3]) {
            i2 = CURRENT_ERA;
        } else {
            ...
            while (i2 < length - 1) {
                int i3 = (i2 + length) / 2;
                int i4 = i3 * 3;
                int iInternalGet2 = iInternalGet - ERAS[i4];
                if (iInternalGet2 == 0 && (iInternalGet2 = internalGet(2) - (ERAS[i4 + 1] - 1)) == 0) {
                    iInternalGet2 = internalGet(5) - ERAS[i4 + 2];
                }
                if (iInternalGet2 >= 0) {
                    i2 = i3;
                } else {
        ...
        internalSet(0, i2);
        internalSet(1, (iInternalGet - ERAS[i2 * 3]) + 1);
    }
