...
abstract class CharsetRecog_mbcs extends CharsetRecognizer {
    ...
    int match(CharsetDetector charsetDetector, int[] iArr) {
        ...
        return Math.min((int) ((Math.log(i4 + 1) * (90.0d / Math.log(i2 / 4.0f))) + 10.0d), 100);
    }
    ...
    static class CharsetRecog_sjis extends CharsetRecog_mbcs {
        ...
        boolean nextChar(iteratedChar iteratedchar, CharsetDetector charsetDetector) {
            ...
            int iNextByte = iteratedchar.nextByte(charsetDetector);
            iteratedchar.charValue = iNextByte;
            if (iNextByte < 0) {
                return false;
            }
            if (iNextByte <= 127 || (iNextByte > 160 && iNextByte <= 223)) {
                return true;
            }
            int iNextByte2 = iteratedchar.nextByte(charsetDetector);
            if (iNextByte2 < 0) {
                return false;
            }
            iteratedchar.charValue = (iNextByte << 8) | iNextByte2;
            if ((iNextByte2 < 64 || iNextByte2 > 127) && (iNextByte2 < 128 || iNextByte2 > 255)) {
        ...
        CharsetMatch match(CharsetDetector charsetDetector) {
            int iMatch = match(charsetDetector, commonChars);
            if (iMatch == 0) {
                return null;
            }
    ...
    static class CharsetRecog_big5 extends CharsetRecog_mbcs {
        ...
        boolean nextChar(iteratedChar iteratedchar, CharsetDetector charsetDetector) {
            ...
            if (iNextByte <= 127 || iNextByte == 255) {
                return true;
            }
            int iNextByte2 = iteratedchar.nextByte(charsetDetector);
            if (iNextByte2 < 0) {
            ...
            iteratedchar.charValue = (iteratedchar.charValue << 8) | iNextByte2;
            if (iNextByte2 < 64 || iNextByte2 == 127 || iNextByte2 == 255) {
                iteratedchar.error = true;
            }
            return true;
        }
    ...
    static abstract class CharsetRecog_euc extends CharsetRecog_mbcs {
        ...
        boolean nextChar(iteratedChar iteratedchar, CharsetDetector charsetDetector) {
            ...
            } else if (iNextByte > 141) {
                int iNextByte2 = iteratedchar.nextByte(charsetDetector);
                iteratedchar.charValue = (iteratedchar.charValue << 8) | iNextByte2;
                if (iNextByte >= 161 && iNextByte <= 254) {
                    if (iNextByte2 < 161) {
                        iteratedchar.error = true;
                    }
                } else if (iNextByte == 142) {
                    if (iNextByte2 < 161) {
                        iteratedchar.error = true;
                    }
                } else if (iNextByte == 143) {
                    int iNextByte3 = iteratedchar.nextByte(charsetDetector);
                    iteratedchar.charValue = (iteratedchar.charValue << 8) | iNextByte3;
                    if (iNextByte3 < 161) {
                        iteratedchar.error = true;
                    }
                }
    ...
    static class CharsetRecog_gb_18030 extends CharsetRecog_mbcs {
        ...
        boolean nextChar(iteratedChar iteratedchar, CharsetDetector charsetDetector) {
            ...
            iteratedchar.error = false;
            int iNextByte3 = iteratedchar.nextByte(charsetDetector);
            iteratedchar.charValue = iNextByte3;
            if (iNextByte3 < 0) {
                iteratedchar.done = true;
            } else if (iNextByte3 > 128) {
                int iNextByte4 = iteratedchar.nextByte(charsetDetector);
                iteratedchar.charValue = (iteratedchar.charValue << 8) | iNextByte4;
                if (iNextByte3 >= 129 && iNextByte3 <= 254 && ((iNextByte4 < 64 || iNextByte4 > 126) && (iNextByte4 < 80 || iNextByte4 > 254))) {
                    if (iNextByte4 >= 48 && iNextByte4 <= 57 && (iNextByte = iteratedchar.nextByte(charsetDetector)) >= 129 && iNextByte <= 254 && (iNextByte2 = iteratedchar.nextByte(charsetDetector)) >= 48 && iNextByte2 <= 57) {
                        iteratedchar.charValue = iNextByte2 | (iteratedchar.charValue << 16) | (iNextByte << 8);
                    } else {
                        iteratedchar.error = true;
                    }
                }
