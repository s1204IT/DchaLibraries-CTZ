...
public class IntTrieBuilder extends TrieBuilder {
    ...
    public int serialize(OutputStream outputStream, boolean z, TrieBuilder.DataManipulate dataManipulate) throws IOException {
        ...
        int i4 = 16 + (this.m_indexLength_ * 2);
        if (z) {
            i2 = i4 + (this.m_dataLength_ * 2);
        } else {
            i2 = i4 + (4 * this.m_dataLength_);
        }
        ...
        if (z) {
            for (int i6 = 0; i6 < this.m_indexLength_; i6++) {
                dataOutputStream.writeChar((this.m_index_[i6] + this.m_indexLength_) >>> 2);
            }
            while (i3 < this.m_dataLength_) {
                dataOutputStream.writeChar(this.m_data_[i3] & DateTimePatternGenerator.MATCH_ALL_FIELDS_LENGTH);
                i3++;
            }
        } else {
    ...
    public boolean setRange(int i, int i2, int i3, boolean z) {
        ...
        int i6 = i2 & 31;
        int i7 = i2 & (-32);
        int dataBlock2 = i3 == this.m_initialValue_ ? 0 : -1;
        while (i4 < i7) {
            int i8 = i4 >> 5;
            int i9 = this.m_index_[i8];
            if (i9 > 0) {
                fillBlock(i9, 0, 32, i3, z);
            } else if (this.m_data_[-i9] != i3 && (i9 == 0 || z)) {
                if (dataBlock2 >= 0) {
                    this.m_index_[i8] = -dataBlock2;
                } else {
                    dataBlock2 = getDataBlock(i4);
                    if (dataBlock2 < 0) {
                        return false;
                    }
                    this.m_index_[i8] = -dataBlock2;
                    fillBlock(dataBlock2, 0, 32, i3, true);
                }
            }
            i4 += 32;
        }
        if (i6 > 0) {
            int dataBlock3 = getDataBlock(i4);
            if (dataBlock3 < 0) {
                return false;
            }
            fillBlock(dataBlock3, 0, i6, i3, z);
        }
        return true;
    }
    ...
    private int getDataBlock(int i) {
        ...
        int iAllocDataBlock = allocDataBlock();
        if (iAllocDataBlock < 0) {
            return -1;
        }
        this.m_index_[i2] = iAllocDataBlock;
        System.arraycopy(this.m_data_, Math.abs(i3), this.m_data_, iAllocDataBlock, 128);
    ...
    private void compact(boolean z) {
        ...
        while (true) {
            ...
            if (this.m_map_[i5] < 0) {
                ...
                if (i3 >= i) {
                    int iFindSameDataBlock = findSameDataBlock(this.m_data_, i4, i3, z ? 4 : 32);
                    if (iFindSameDataBlock >= 0) {
                        this.m_map_[i5] = iFindSameDataBlock;
                        i3 += 32;
                    }
                }
                if (z && i3 >= i) {
                    i2 = 28;
                    while (i2 > 0 && !equal_int(this.m_data_, i4 - i2, i3, i2)) {
                        i2 -= 4;
                    }
                }
                if (i2 > 0) {
                    this.m_map_[i5] = i4 - i2;
                    i3 += i2;
                    int i6 = 32 - i2;
                    while (i6 > 0) {
                        this.m_data_[i4] = this.m_data_[i3];
                        i6--;
                        i4++;
                        i3++;
                    }
                    ...
                    int i8 = 32;
                    while (i8 > 0) {
                        this.m_data_[i4] = this.m_data_[i7];
                        i8--;
                        i4++;
                        i7++;
                    }
    ...
    private final void fold(TrieBuilder.DataManipulate dataManipulate) {
        ...
        if (this.m_leadUnitValue_ != this.m_initialValue_) {
            ...
            fillBlock(iAllocDataBlock, 0, 32, this.m_leadUnitValue_, true);
            i = -iAllocDataBlock;
        } else {
            i = 0;
        }
        ...
        while (i3 < 1114112) {
            if (iArr2[i3 >> 5] != 0) {
                int i5 = i3 & (-1024);
                int i6 = i5 >> 5;
                int iFindSameIndexBlock = findSameIndexBlock(iArr2, i4, i6);
                int foldedValue = dataManipulate.getFoldedValue(i5, iFindSameIndexBlock + 32);
