...
public class VectorDrawableCompat extends VectorDrawableCommon {
    ...
    VectorDrawableCompat(VectorDrawableCompatState state) {
        ...
        PorterDuff.Mode mode = state.mTintMode;
        throw this;
    }
        ...
        private void drawGroupTree(VGroup currentGroup, Matrix currentMatrix, Canvas canvas, int w, int h, ColorFilter filter) {
            ...
            while (true) {
                ...
                if (i2 < currentGroup.mChildren.size()) {
                    ?? r10 = currentGroup.mChildren.get(i2);
                    if (r10 instanceof VGroup) {
                        drawGroupTree(r10, currentGroup.mStackedMatrix, canvas, w, h, filter);
                    } else if (r10 instanceof VPath) {
                        drawPath(currentGroup, r10, canvas, w, h, filter);
                    }
                    i = i2 + 1;
                } else {
                    canvas.restore();
    ...
    private static class VGroup {
        ...
        public VGroup(VGroup copy, ArrayMap<String, Object> targetsMap) {
            ...
            for (int i = 0; i < children.size(); i++) {
                ?? r2 = children.get(i);
                if (r2 instanceof VGroup) {
                    this.mChildren.add(new VGroup(r2, targetsMap));
                } else {
                    if (r2 instanceof VFullPath) {
                        newPath = new VFullPath(r2);
                    } else if (r2 instanceof VClipPath) {
                        newPath = new VClipPath(r2);
                    } else {
