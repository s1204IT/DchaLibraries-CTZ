...
public class TabLayout extends HorizontalScrollView {
    ...
    private void updateTabViewLayoutParams(LinearLayout.LayoutParams lp) {
        if (this.mode == 1 && this.tabGravity == 0) {
            ((ViewGroup.LayoutParams) lp).width = 0;
            lp.weight = 1.0f;
        } else {
        ...
        private void updateBackgroundDrawable(Context context) {
            ...
            if (TabLayout.this.tabRippleColorStateList != null) {
                ...
                if (Build.VERSION.SDK_INT >= 21) {
                    background = new RippleDrawable(rippleColor, TabLayout.this.unboundedRipple ? null : gradientDrawable, TabLayout.this.unboundedRipple ? null : maskDrawable);
                } else {
                    Drawable rippleDrawable = DrawableCompat.wrap(maskDrawable);
                    DrawableCompat.setTintList(rippleDrawable, rippleColor);
                    background = new LayerDrawable(new Drawable[]{gradientDrawable, rippleDrawable});
                }
            } else {
    ...
    private class SlidingTabIndicator extends LinearLayout {
            ...
            if (View.MeasureSpec.getMode(widthMeasureSpec) == 1073741824 && TabLayout.this.mode == 1 && TabLayout.this.tabGravity == 1) {
                ...
                if (largestTabWidth * count <= getMeasuredWidth() - (gutter * 2)) {
                    while (true) {
                        ...
                        if (((ViewGroup.LayoutParams) lp).width != largestTabWidth || lp.weight != 0.0f) {
                            ((ViewGroup.LayoutParams) lp).width = largestTabWidth;
                            lp.weight = 0.0f;
                            remeasure = true;
                        }
                        i2 = i3 + 1;
        ...
        private void updateIndicatorPosition() {
            ...
            if (childAt != 0 && childAt.getWidth() > 0) {
                ...
                if (!TabLayout.this.tabIndicatorFullWidth && (childAt instanceof TabView)) {
                    calculateTabViewContentBounds(childAt, TabLayout.this.tabViewContentBounds);
                    left = (int) TabLayout.this.tabViewContentBounds.left;
                    right = (int) TabLayout.this.tabViewContentBounds.right;
                }
                if (this.selectionOffset > 0.0f && this.selectedPosition < getChildCount() - 1) {
                    ?? childAt2 = getChildAt(this.selectedPosition + 1);
                    int nextTitleLeft = childAt2.getLeft();
                    int nextTitleRight = childAt2.getRight();
                    if (!TabLayout.this.tabIndicatorFullWidth && (childAt2 instanceof TabView)) {
                        calculateTabViewContentBounds(childAt2, TabLayout.this.tabViewContentBounds);
        ...
        void animateIndicatorToPosition(final int position, int duration) {
            ...
            ?? childAt = getChildAt(position);
            if (childAt == 0) {
                updateIndicatorPosition();
                return;
            }
            int targetLeft = childAt.getLeft();
            int targetRight = childAt.getRight();
            if (!TabLayout.this.tabIndicatorFullWidth && (childAt instanceof TabView)) {
                calculateTabViewContentBounds(childAt, TabLayout.this.tabViewContentBounds);
