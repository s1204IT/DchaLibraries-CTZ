...
public class MtkIPv6TetheringCoordinator extends IPv6TetheringCoordinator {
    private static final boolean DBG = true;
    private static final String TAG = MtkIPv6TetheringCoordinator.class.getSimpleName();
    ...
    public MtkIPv6TetheringCoordinator(ArrayList<TetherInterfaceStateMachine> arrayList, SharedLog sharedLog) {
        this.mNotifyList = arrayList;
        this.mLog = sharedLog.forSubComponent(TAG);
        Log.d(TAG, "MtkIPv6TetheringCoordinator");
    }
    ...
    private void updateIPv6TetheringInterfaces() {
        ...
        if (it.hasNext()) {
            ...
            next.sendMessage(327793, 0, 0, getInterfaceIPv6LinkProperties(next));
        }
    }
    ...
    private LinkProperties getInterfaceIPv6LinkProperties(TetherInterfaceStateMachine tetherInterfaceStateMachine) {
        ...
        if (this.mUpstreamNetworkState != null && this.mUpstreamNetworkState.linkProperties != null && (downstreamPeek = this.mActiveDownstreams.peek()) != null && downstreamPeek.tism == tetherInterfaceStateMachine) {
            ...
            if (iPv6OnlyLinkProperties.hasIPv6DefaultRoute() && hasGlobalIPv6Address(iPv6OnlyLinkProperties)) {
    ...
    private static boolean canTetherIPv6(NetworkState networkState, SharedLog sharedLog) {
        ...
        boolean z2 = (routeInfo2 == null || routeInfo == null || routeInfo2.getInterface() == null || !routeInfo2.getInterface().equals(routeInfo.getInterface())) ? false : true;
        boolean z3 = z && z2;
        if (!z3 && routeInfo2 == null && routeInfo != null) {
            Log.d(TAG, "Use IPv6 only");
        } else {
            if (!z3 && networkState != null && networkState.linkProperties != null) {
                Log.d(TAG, "Check clatd:" + z + ":" + z2);
                if (networkState.linkProperties.getStackedLinks().size() > 0) {
                    Log.d(TAG, "Bypass clatd check");
                }
            }
            if (networkState == null) {
                Object[] objArr = new Object[2];
                objArr[0] = z3 ? "available" : "not available";
    ...
    private static LinkProperties getIPv6OnlyLinkProperties(LinkProperties linkProperties) {
        ...
        for (LinkAddress linkAddress : linkProperties.getLinkAddresses()) {
            if (isIPv6GlobalAddress(linkAddress.getAddress()) && linkAddress.getPrefixLength() == 64) {
    ...
    private static void stopIPv6TetheringOn(TetherInterfaceStateMachine tetherInterfaceStateMachine) {
        tetherInterfaceStateMachine.sendMessage(327793, 0, 0, (Object) null);
    }
    ...
    private static boolean hasGlobalIPv6Address(LinkProperties linkProperties) {
        ...
        for (InetAddress inetAddress : linkProperties.getAddresses()) {
            if ((inetAddress instanceof Inet6Address) && isIPv6GlobalAddress(inetAddress)) {
                return true;
            }
        }
        return false;
