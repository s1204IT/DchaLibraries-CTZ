...
public class CarrierConfigManager {
    ...
    static {
        ...
        sDefaults.putStringArray(KEY_CARRIER_METERED_IWLAN_APN_TYPES_STRINGS, new String[]{PhoneConstants.APN_TYPE_DEFAULT});
        ...
        sDefaults.putIntArray(KEY_LTE_RSRP_THRESHOLDS_INT_ARRAY, new int[]{WifiNetworkScoreCache.INVALID_NETWORK_SCORE, -118, PackageManager.INSTALL_PARSE_FAILED_MANIFEST_MALFORMED, -98});
    ...
    public PersistableBundle getConfigForSubId(int i) {
        try {
            ...
            if (iCarrierConfigLoader == null) {
                Rlog.w(TAG, "Error getting config for subId " + i + " ICarrierConfigLoader is null");
                return null;
            }
            return iCarrierConfigLoader.getConfigForSubId(i);
        } catch (RemoteException e) {
            Rlog.e(TAG, "Error getting config for subId " + i + ": " + e.toString());
            return null;
        }
    }
    ...
    public void notifyConfigChangedForSubId(int i) {
        try {
            ICarrierConfigLoader iCarrierConfigLoader = getICarrierConfigLoader();
            if (iCarrierConfigLoader == null) {
                Rlog.w(TAG, "Error reloading config for subId=" + i + " ICarrierConfigLoader is null");
                return;
            }
            iCarrierConfigLoader.notifyConfigChangedForSubId(i);
        } catch (RemoteException e) {
            Rlog.e(TAG, "Error reloading config for subId=" + i + ": " + e.toString());
        }
    }
    ...
    public void updateConfigForPhoneId(int i, String str) {
        try {
            ICarrierConfigLoader iCarrierConfigLoader = getICarrierConfigLoader();
            if (iCarrierConfigLoader == null) {
                Rlog.w(TAG, "Error updating config for phoneId=" + i + " ICarrierConfigLoader is null");
                return;
            }
            iCarrierConfigLoader.updateConfigForPhoneId(i, str);
        } catch (RemoteException e) {
            Rlog.e(TAG, "Error updating config for phoneId=" + i + ": " + e.toString());
        }
    }
