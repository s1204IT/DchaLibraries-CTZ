...
public abstract class CameraMetadata<TKey> {
    ...
    <TKey> ArrayList<TKey> getKeys(Class<?> cls, Class<TKey> cls2, CameraMetadata<TKey> cameraMetadata, int[] iArr) {
        ...
        GridLayout.Assoc assoc = (ArrayList<TKey>) new ArrayList();
        ...
        if (this.mNativeInstance != null && (allVendorKeys = this.mNativeInstance.getAllVendorKeys(cls2)) != null) {
            for (Object obj2 : allVendorKeys) {
                if (obj2 instanceof CaptureRequest.Key) {
                    CaptureRequest.Key key = (CaptureRequest.Key) obj2;
                    name = key.getName();
                    vendorId = key.getVendorId();
                } else if (obj2 instanceof CaptureResult.Key) {
                    CaptureResult.Key key2 = (CaptureResult.Key) obj2;
                    name = key2.getName();
                    vendorId = key2.getVendorId();
                } else if (obj2 instanceof CameraCharacteristics.Key) {
                    CameraCharacteristics.Key key3 = (CameraCharacteristics.Key) obj2;
                    name = key3.getName();
    ...
    private static <TKey> boolean shouldKeyBeAdded(TKey tkey, Field field, int[] iArr) {
        ...
        return iArr == null || field.getAnnotation(SyntheticKey.class) != null || Arrays.binarySearch(iArr, nativeKey.getTag()) >= 0;
    }
}
