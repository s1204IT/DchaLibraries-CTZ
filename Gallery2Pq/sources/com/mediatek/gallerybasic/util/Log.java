...
public class Log {
    ...
    private static final int LOG_LEVEL_IN_PROPERTY = SystemPropertyUtils.getInt("vendor.debug.gallery.loglevel", 2);

    static {
        int i = 2;
        if (LOG_LEVEL_IN_PROPERTY >= 0 && LOG_LEVEL_IN_PROPERTY <= CUST_LOG_LEVEL_E) {
    ...
    private static boolean enableCustLog(int i) {
        return CUST_LOG_LEVEL >= 0 && CUST_LOG_LEVEL <= CUST_LOG_LEVEL_E && i >= CUST_LOG_LEVEL;
    }
}
