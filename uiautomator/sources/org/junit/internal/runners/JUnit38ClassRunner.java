...
public class JUnit38ClassRunner extends Runner implements Filterable, Sortable {
    ...
    private static final class OldTestClassAdaptingListener implements TestListener {
        ...
        private String getName(Test test) {
            return test instanceof TestCase ? test.getName() : test.toString();
        }
    ...
    private static Description makeDescription(Test test) {
        if (test instanceof TestCase) {
            return Description.createTestDescription(test.getClass(), test.getName(), getAnnotations(test));
        }
        if (test instanceof TestSuite) {
            Description descriptionCreateSuiteDescription = Description.createSuiteDescription(test.getName() == null ? createSuiteDescription(test) : test.getName(), new Annotation[0]);
            int iTestCount = test.testCount();
            for (int i = 0; i < iTestCount; i++) {
                descriptionCreateSuiteDescription.addChild(makeDescription(test.testAt(i)));
            }
        ...
        if (test instanceof TestDecorator) {
            return makeDescription(test.getTest());
        }
